    ...
    handlePath = () => {
        ...
        if (this.path != path) {
            ...
            if (this.path === "" || this.path === null) {
                ...
                let hasHttp = this.path.indexOf('http') !== -1 
                let returnPath = hasHttp ? this.path : resourceUrlPathResize ? `${resourceUrlPathResize.textValue}=${this.path}`: ''
                this.state._path = returnPath
                if (!Utils.isNull(resizeAtt) && !hasHttp) {
